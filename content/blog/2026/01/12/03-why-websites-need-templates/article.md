...
When I load a web page in a browser, what arrives is a single document. It might be long, it might have navigation, footers, sidebars, and a main column of text, but to the browser it is just one block of HTML.

When I run a site with more than a handful of pages, however, I am not authoring one document. I am authoring many. Each page has its own content, but the structure repeats. The header, the navigation, the footer, the typography, and the basic layout are usually the same everywhere.
...
A template lets me write the shared structure once and reuse it. I define a common outer document — the `<html>`, `<head>`, header, navigation, footer, and so on — and I leave one or more places for page-specific content. Each article or page then supplies its own content, and the build places it into those slots.
...
Where templates get more complicated is in how much responsibility I give them. Many systems ask templates to do more than place content into a frame. They ask them to decide which pieces of content should appear, in what order, and under what conditions. That turns the template into a control layer with decisions baked in.
...
In the next article I look at how that additional responsibility changes what templates are, and why it affects the clarity and predictability of the final HTML. If you want to continue, head to [When Templates Start Making Decisions](/content/blog/2026/01/12/04-when-templates-become-programs/).
