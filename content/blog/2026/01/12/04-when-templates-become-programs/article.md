...
_January 12, 2026_ | Series: templating | Tags: templating, publishing, process, tooling
...
At that point the template no longer just inserts a finished piece of content into a slot. It gets access to a collection of articles, tags, or metadata. It then loops over those items, filters them, sorts them, and decides whether certain blocks of HTML should appear. That shift makes it responsible for structure, selection, and ordering.

At a small scale this feels convenient. If I want a list of recent posts in a sidebar, it seems reasonable to write a loop in the template that iterates over the latest five articles, and I add a conditional to hide a heading when there are no results. Over time, more of these small decisions accumulate.
...
This has a few practical consequences. I notice them most when I try to read a page without running the build.

One is that the template stops being a reliable description of the document. I can open it in an editor and see a mix of HTML and logic, but I cannot easily tell which sections will appear in the output for a given build. I might wrap a block in a conditional. A list might be empty. A heading might only appear if a certain field exists. The structure of the page is no longer fixed.
