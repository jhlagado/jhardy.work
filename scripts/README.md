...
This runs the lint report, builds the site, starts the local server, and rebuilds on changes in `content/`, `config/`, and `assets/`. By default the dev server binds to `127.0.0.1`; set `HOST=0.0.0.0` if you need to reach it from another device, and override the port with `PORT=xxxx` if needed.
...
The prose linter scans `content/**/article.md`.
