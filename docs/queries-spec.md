...
* required
* string
* currently must be `"blog"` (maps to the active content root; default `content/semantic-scroll/` in this repo, or `content/<contentDir>/` when `site-config.json` overrides it)
