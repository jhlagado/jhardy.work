...
The identity of an article is rooted in the filesystem, which serves as the sole authoritative source for its chronological position and unique identity. We use a structured path hierarchy: `/content/<contentDir>/YYYY/MM/DD/NN-slug/article.md` (default `content/semantic-scroll/` in this repo, or the `contentDir` specified in `site-config.json`).
