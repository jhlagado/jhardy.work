# AI-Driven Blogging Setup: Product Requirements (Cleaned)

This section is the authoritative, cleaned version of the spec. The original draft transcript is preserved below for traceability and remains verbatim.
...
- [5. Draft Lifecycle](#5-draft-lifecycle)
- [6. Metadata and Tagging](#6-metadata-and-tagging)
- [7. Internal Linking](#7-internal-linking)
- [8. Asset Management](#8-asset-management)
- [9. Review Gate](#9-review-gate)
- [10. Authoring Vocabulary](#10-authoring-vocabulary)
- [11. CI and Publishing Process](#11-ci-and-publishing-process)
- [12. CSS and Styling](#12-css-and-styling)
- [Appendix: Original Draft Transcript (Verbatim)](#appendix-original-draft-transcript-verbatim)

## 1. Vision and Goals
- Serve as a primary record of technical work, decisions, and experiments.
- Use AI to accelerate drafting while keeping human editorial control.
- Preserve classic web values: semantic HTML, accessibility, minimal JavaScript, and fast pages.
- Keep content durable, portable, and easy to maintain over time.

## 2. Design Principles
- **Semantic HTML**: structured tags that read well for humans and machines.
- **Accessibility**: keyboard navigation, alt text, ARIA where needed, and strong contrast.
- **Minimal JavaScript**: progressive enhancement only; no heavy frameworks.
- **Performance and SEO**: fast page loads, optimized assets, crawlable structure.
- **Mobile-first**: design for small screens first, then scale up.

## 3. Content Model
- Each post lives in `/blog/YYYY/MM/DD/slug/` with its markdown file and co-located assets.
- Frontmatter fields include `title`, `date`, `status`, `summary`, and `tags`.
- `summary` should be a short, 1-2 sentence abstract used in listings and previews.

## 4. Authoring Workflow
- **Conversational drafting**: AI suggests title, summary, tags, and status from a dialogue.
- **Folder-based output**: create the post folder, markdown file, and referenced assets together.
- **Script evolution**: repeated tasks become scripts stored in `/scripts/`.
...
- Normalize punctuation variants (for example, `z80`, `Z-80`, `Z_80`).

## 7. Internal Linking
- Use stable, relative paths rather than hard-coded URLs.
...
- Human-driven review with optional AI checks.

## 10. Authoring Vocabulary
- Core verbs: `create`, `revise`, `tag`, `link`, `attach`, `status`.
- Verbs map to simple, scriptable actions.

## 11. CI and Publishing Process
- CI converts markdown to HTML using templates and deploys to `gh-pages`.
- Publish triggers on commit push or PR merge.
- Index pages are generated by tag and date.
- The build copies assets and flags missing metadata or unresolved links.
- Tooling should remain lean, favoring lightweight tools and small custom scripts.
...
- Avoid large CSS frameworks; prefer native CSS features.
- Keep CSS lightweight for fast loads.

---

## Appendix: Original Draft Transcript (Verbatim)
